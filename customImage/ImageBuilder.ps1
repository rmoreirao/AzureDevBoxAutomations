...
function Start-CustomImageBuild {
    ...
    # Create the image definition with custom publisher info
    New-AzGalleryImageDefinition -GalleryName $galleryName -ResourceGroupName $imageResourceGroup -Location $location -Name $imageDefName -OsState generalized -OsType Windows -Publisher 'CustomOrg' -Offer 'devbox-custom' -Sku '2024-1' -Feature $features -HyperVGeneration "V2"
